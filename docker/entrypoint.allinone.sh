#!/bin/bash
# FreeCash Solo – freecashd + stratum + dashboard (5050) with auto-restart
set -u
...
JOB_INTERVAL="${FCH_JOB_INTERVAL:-4}"
...
p=Path('/app/config/config.yaml'); cfg={}
if p.exists():
    try: cfg=yaml.safe_load(p.read_text()) or {}
    except Exception: cfg={}
cfg.setdefault('rpc',{}); cfg['rpc'].update({'host':'127.0.0.1','port':int(os.environ.get('FCH_RPCPORT','8332')),'user':os.environ.get('FCH_RPCUSER','fchrpc'),'password':os.environ.get('FCH_RPCPASS','FreecashSoloAutoRpc_ChangeMeIfPublic'),'timeout':30})
cfg.setdefault('pool',{}); cfg['pool'].setdefault('payout_address','FCHANGE_ME_GETNEWADDRESS'); cfg['pool'].setdefault('stratum_port',3333); cfg['pool'].setdefault('stratum_host','0.0.0.0'); cfg['pool'].setdefault('start_difficulty',256); cfg['pool']['job_interval']=int(os.environ.get('FCH_JOB_INTERVAL','4'))
...
print('[allinone] job interval',cfg['pool']['job_interval'],'s; monitor port',cfg['monitor']['port'])
PY

if [ -x /usr/local/bin/freecash-cli ] && [ ! -f /usr/local/bin/freecash-cli.real ]; then mv /usr/local/bin/freecash-cli /usr/local/bin/freecash-cli.real; fi
...
for i in $(seq 1 180); do
  if freecash-cli getblockchaininfo >/dev/null 2>&1; then echo "[allinone] RPC ok (${i}s)"; break; fi
  if ! kill -0 $NODE_PID 2>/dev/null; then echo "[allinone] freecashd died"; exit 1; fi
  sleep 1
done
...
run_forever() {
  ...
  while true; do
    ...
    wait $pid; local rc=$?
    echo "[allinone] $name exited rc=$rc – restart in 3s"; sleep 3
  done
}
...
echo "[allinone] Job poll   ${JOB_INTERVAL}s"
echo "[allinone] ========================================"
wait $NODE_PID
