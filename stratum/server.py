#!/usr/bin/env python3
"""
Production Solo Stratum – AxeOS / NerdQaxe++ kompatibel
Portiert aus fch-node (funktionierender Share-Stack).

- coinbase = coinb1 + en1 + en2 + coinb2
- Version-Rolling: (job & ~mask) | (submitted & mask)
- prevhash notify = reverse WORD ORDER of BE (NerdQaxe swap_endian_words)
- header validation uses LE prevhash = full byte-reverse of BE
- schreibt data/stats.json für das Dashboard
"""
...
log = logging.getLogger("solo-stratum")
    ...
    try:
        if STATS_PATH.exists():
            ...
            for k, v in data.items():
                if k in _stats:
                    _stats[k] = v
            if not _stats.get("started_at"):
                _stats["started_at"] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    except Exception:
        pass
...
def stratum_prevhash(rpc_be_hex: str) -> str:
    """Stratum prevhash for NerdQaxe/ckpool:
    Reverse ORDER of 8x4-byte words of BE hash.
    """
    h = rpc_be_hex.lower()
    if len(h) != 64:
...
def address_to_scriptpubkey(addr: str) -> bytes:
    ...
    h160 = decoded[1:21]
    if len(h160) != 20:
        raise ValueError("bad hash160 from address")
...
class Client(threading.Thread):
    ...
    def handle_submit(self, mid, params):
        if len(params) < 5:
            ...
            self.shares_bad += 1
            _bump_worker(self.worker, ok=False)
            _save_stats()
            return
        ...
        self.send({"id": mid, "result": True, "error": None})
        self.shares_ok += 1
        share_diff = target_to_difficulty(h_int)
        with _stats_lock:
            ...
            if share_diff > (_stats.get("best_share_diff") or 0):
                _stats["best_share_diff"] = share_diff
...
def main():
    ...
    _save_stats()
    log.info("=" * 50)
    log.info("Solo Stratum (NerdQaxe / AxeOS compatible)")
